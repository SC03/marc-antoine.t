...
public class Intro extends ApplicationAdapter {
  ...
  int longueurFenetre;
  int hauteurFenetre;
  ArrayList<Sprite> liste_sprites;
  Joueur joueur;
  boolean gameOver;
  ...
  private void initialisationSprites() {
    liste_sprites = new ArrayList<>();
    for (int i = 0; i < NB_SPRITES; i++) {
      liste_sprites.add(new Sprite ("chien.png"));
    }
  }
  ...
  private void majEtatProtagonistes() {
    // Sprites
    for (Sprite sprite : liste_sprites) {
    sprite.majEtat();
    }
  ...
  private void majEtatJeu() {
    ...
    if (joueur.estEnCollisionAvec(liste_sprites)) {
      gameOver = true;
    }
  }
